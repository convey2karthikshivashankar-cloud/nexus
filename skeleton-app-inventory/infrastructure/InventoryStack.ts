...
export class InventoryStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const eventStore = new dynamodb.Table(this, 'EventStore', {
      tableName: `skeleton-inventory-events-${accountId}`,
    ...
    const readModel = new dynamodb.Table(this, 'ReadModel', {
      tableName: `skeleton-inventory-readmodel-${accountId}`,
    ...
    const eventBus = new events.EventBus(this, 'EventBus', {
      eventBusName: `skeleton-inventory-bus-${accountId}`,
    });

    const commandHandler = new lambda.Function(this, 'CommandHandler', {
      functionName: `skeleton-inventory-cmd-${accountId}`,
    ...
    const queryHandler = new lambda.Function(this, 'QueryHandler', {
      functionName: `skeleton-inventory-query-${accountId}`,
      runtime: lambda.Runtime.NODEJS_18_X,
      handler: 'index.handler',
    ...
    const eventProcessor = new lambda.Function(this, 'EventProcessor', {
      functionName: `skeleton-inventory-proc-${accountId}`,
      runtime: lambda.Runtime.NODEJS_18_X,
      handler: 'index.handler',
    ...
    const api = new apigateway.RestApi(this, 'InventoryApi', {
      restApiName: `skeleton-inventory-api-${accountId}`,
      defaultCorsPreflightOptions: {
        allowOrigins: apigateway.Cors.ALL_ORIGINS,
        allowMethods: apigateway.Cors.ALL_METHODS,
      },
    });
    api.root.addResource('command').addMethod('POST', new apigateway.LambdaIntegration(commandHandler));
    api.root.addResource('query').addMethod('GET', new apigateway.LambdaIntegration(queryHandler));
    api.root.addResource('events').addMethod('GET', new apigateway.LambdaIntegration(queryHandler));

    const uiBucket = new s3.Bucket(this, 'UIBucket', {
      bucketName: `skeleton-inventory-ui-${accountId}`,
