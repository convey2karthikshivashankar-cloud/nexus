...
import { GitBranch, RefreshCw, Clock, Hash, FileJson, Zap, ChevronDown, ChevronRight, Database, Filter, Calendar, ArrowUpDown, Activity, Layers, Thermometer, AlertTriangle, CheckCircle } from 'lucide-react'
...
export default function EventTimeline({ events, onRefresh }: Props) {
  ...
  const getEventStyle = (type: string) => {
    switch (type) {
      case 'SensorRegistered': return { bg: 'rgba(34, 197, 94, 0.15)', color: '#22c55e', border: 'rgba(34, 197, 94, 0.3)', icon: <Thermometer size={12} /> }
      case 'ReadingRecorded': return { bg: 'rgba(6, 182, 212, 0.15)', color: '#06b6d4', border: 'rgba(6, 182, 212, 0.3)', icon: <Activity size={12} /> }
      case 'AlertTriggered': return { bg: 'rgba(249, 115, 22, 0.15)', color: '#f97316', border: 'rgba(249, 115, 22, 0.3)', icon: <AlertTriangle size={12} /> }
      case 'AlertAcknowledged': return { bg: 'rgba(168, 85, 247, 0.15)', color: '#a855f7', border: 'rgba(168, 85, 247, 0.3)', icon: <CheckCircle size={12} /> }
      default: return { bg: 'rgba(168, 85, 247, 0.15)', color: '#a855f7', border: 'rgba(168, 85, 247, 0.3)', icon: <Zap size={12} /> }
    }
  }
  ...
  const stats = useMemo(() => ({
    total: events.length,
    registered: events.filter(e => e.eventType === 'SensorRegistered').length,
    readings: events.filter(e => e.eventType === 'ReadingRecorded').length,
    alerts: events.filter(e => e.eventType === 'AlertTriggered').length,
      ...
      <div style={{ background: 'linear-gradient(135deg, rgba(168, 85, 247, 0.15), rgba(236, 72, 153, 0.1))', borderBottom: '1px solid rgba(168, 85, 247, 0.2)', padding: '24px 28px' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '20px', marginBottom: '20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            <div>
              <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'white', marginBottom: '4px' }}>IoT Event Stream</h2>
              <p style={{ color: 'rgba(255, 255, 255, 0.5)', fontSize: '0.9rem' }}>Immutable sensor event log • {events.length} events</p>
            </div>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '12px' }}>
          ...
          <MiniStat label="Registered" value={stats.registered} color="#22c55e" />
          <MiniStat label="Readings" value={stats.readings} color="#06b6d4" />
          <MiniStat label="Alerts" value={stats.alerts} color="#f97316" />
      ...
      <div style={{ padding: '16px 24px', borderBottom: '1px solid rgba(255, 255, 255, 0.06)', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <div style={{ display: 'flex', background: 'rgba(255,255,255,0.05)', borderRadius: '10px', padding: '4px' }}>
            ...
            <ViewButton active={viewMode === 'grouped'} onClick={() => setViewMode('grouped')} icon={<Layers size={14} />} label="By Sensor" />
          ...
          return (
            <div key={event.eventId}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 24px', borderBottom: '1px solid rgba(255, 255, 255, 0.04)', background: index % 2 === 0 ? 'transparent' : 'rgba(255, 255, 255, 0.01)' }}>
                ...
                <div style={{ flex: 2 }}><span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '5px 10px', borderRadius: '6px', fontWeight: 700, fontSize: '0.8rem', background: style.bg, border: `1px solid ${style.border}`, color: style.color }}>{style.icon} {event.eventType}</span></div>
    ...
    <div style={{ background: 'rgba(255, 255, 255, 0.02)', border: `1px solid ${style.border}`, borderRadius: '12px', overflow: 'hidden' }}>
      <div style={{ padding: compact ? '12px 16px' : '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flex: 1 }}>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '5px 10px', borderRadius: '6px', fontWeight: 700, fontSize: '0.8rem', background: style.bg, border: `1px solid ${style.border}`, color: style.color }}>{style.icon} {event.eventType}</span>
...
function EmptyState() {
  return (
    <div style={{ textAlign: 'center', padding: '60px 24px' }}>
      ...
      <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '0.9rem' }}>Register sensors and record readings to see events</p>
    </div>
  )
}
